...
export const addCart=async(req,res)=>{
  try {
    ...
    if (!cart) {
      cart=new cartEntity({
        clientId:idClient,
        products:[{productId:productId,productName:productName,price:numericPrice,quantity:1,color:"-"}],
      })
      await cart.save();
    } else {
      ...
      if (productIndex>-1) {
        ...
        cart.products.push({productId:productId,productName:productName,price:numericPrice,quantity:1,color:"-"})
      }
      cart.updateAt=new Date();
      await cart.save();
    }
    const totalItems=cart.products.reduce((sum,item)=>sum+item.quantity,0);
    res.json({mess:`Đã thêm phần mềm ${productName} vào giỏ hàng`,success:true,totalItems:totalItems});
  } catch (error) {
    const {productName}=req.body;
    res.json({mess:`Thêm phần mềm ${productName} vào giỏ hàng thất bại`,success:false,error:error.message});
  }
}
