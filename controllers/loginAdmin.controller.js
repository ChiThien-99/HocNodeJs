...
export const postLoginAdmin = async (req, res) => {
  try {
    ...
    const admin = await adminEntity.findOne({ email: emailAdmin });
    if (!admin || !(await bcrypt.compare(pwAdmin, admin.password))) {
      return res
        .status(401)
        .json({ mess: "Sai thông tin đăng nhập", success: false });
    }
    res.status(200).json({ idAdmin: admin._id, success: true });
  } catch (error) {
    console.error(error);
